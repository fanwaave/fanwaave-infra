...
| Path | Purpose |
| --- | --- |
| `neon/fanwaave-prod/terraform/` | pinned `kislerdm/neon` provider + lock file; project `fanwaave-prod` in `aws-us-east-2` with `prevent_destroy` |
...
State: **planned**. No fanwaave Neon organization or project has been provider-verified yet, so no ID in this repo is
...
- Migrations are authored in `fanwaave/fanwaave-lib-core` and applied by `dpm`; this repository is not a migration authority.
